...
- New workflow `.github/workflows/npm-publish.yml`, triggered `push: tags: ['v*']` — modeled directly on `nuget-publish.yml`'s existing pattern:
  ...
  - `dotnet publish src/WasmPlayer/WasmPlayer.csproj -c Release` (same command `deploy-play.yml` already runs). This duplicates ~15s of build time in a second parallel tag-triggered job rather than adding cross-workflow artifact sharing — consistent with how `nuget-publish.yml`/`deploy-play.yml`/`electron-publish.yml` already run as independent parallel jobs off the same tag (see root `CLAUDE.md`'s Releasing section).
...
- Generated `index.html` head starts with `<base href="https://cdn.jsdelivr.net/npm/@textadventures/quest-viva-wasmplayer@<pinned-version>/">`, before any other resource tag. Per URL-resolution spec, this single line:
  ...
  - **Correction found during implementation (empirical testing caught this — see below):** the original assumption here — that `wasm-player.js:436`'s `import('./_framework/dotnet.js')` would resolve against the script's own URL regardless of `<base href>`, unaffected by cross-origin loading — is **wrong**. When `wasm-player.js` is loaded cross-origin (as it now is, from the CDN) as a classic (non-module) script, Chromium has no usable base URL for a *relative* specifier passed to a dynamic `import()` call inside it, and fails outright with `TypeError: Failed to resolve module specifier './_framework/dotnet.js'. The base URL is about:blank because import() is called from a CORS-cross-origin script.` Adding `crossorigin="anonymous"` to the `<script>` tag does **not** fix this (also confirmed empirically). The actual fix, now implemented: `wasm-player.js` captures its own absolute URL synchronously via `document.currentScript?.src` (only valid during a classic script's initial top-level execution) into a module-scoped `wasmPlayerScriptUrl` const near the top of the file, then builds an absolute URL with `new URL('_framework/dotnet.js', wasmPlayerScriptUrl).href` and passes *that* to `import()` instead of the relative specifier — sidestepping relative-specifier resolution (and its cross-origin quirk) entirely. Harmless for the normal same-origin deployment too (resolves to the same URL either way).
- A `<script>window.QuestVivaEmbeddedGame = "<base64>";</script>` before the `wasm-player.js` tag, plus a sibling `window.QuestVivaEmbeddedGameFilename` (the original file's basename) — its extension is what `ByteArrayGameDataProvider`/`WasmPlayerBridge.Initialise` use to pick `.quest` zip-package vs. plain `.aslx` vs. Legacy `.asl`/`.cas` parsing, so it has to survive the export, not just the bytes. No `quest-config.js` needed in this flavor — nothing else is local.
- **First implementation step should be to verify the `<base href>` reasoning empirically** against a real browser (build one by hand, serve over `http://` via e.g. `npx serve`, confirm via network-request inspection that every asset actually resolves to the CDN and nothing 404s locally) before wiring it into a generated build step — this is a correct-per-spec but fairly obscure corner of URL resolution, worth confirming directly rather than trusting the reasoning alone. **This caught the dynamic-import bug above** — done by serving a real AppBundle build as a CDN stand-in on one local port (with `Access-Control-Allow-Origin: *`, matching what jsDelivr actually serves) and the generated single-file export from a different port, confirming via console/network inspection that a `.quest` package and a plain `.aslx` game both boot with zero requests to the export's own origin beyond the HTML shell itself.
