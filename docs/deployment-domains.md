...
- **Play tab**: category list (Top 20 / Newest Games / per-tag — "New uploads" was dropped server-side, see above) fetched from `api/Catalog` above, with `source`/`version`/`platform` client info attached (`version` read at runtime via `fetch('VERSION')` — the repo-root `VERSION` file is copied alongside Home's static assets in `deploy-play.yml`; `source` detected via `typeof window.electronApp !== 'undefined'`, the same check `WebEditor`'s `runtime.ts` uses). Each game card links to `/player/?id={id}` — WasmPlayer already handles `?id=` end-to-end, so no player-side work was needed.
