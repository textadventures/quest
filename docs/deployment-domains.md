...
- **Root (`/`)** — Play tab (game catalog) when nothing's loaded, the editor canvas once a game is (same `isLoaded` check as before, just a different "nothing loaded" screen). Gated by a new `PUBLIC_SHOW_HOME` build-time flag (`src/WebEditor/.env`, default `false`) — `true` for play.questviva.com and Electron, unset for textadventures.co.uk's `/questviva` build, which keeps its previous editor-only root behavior (that origin has its own site navigation, not a game-browser homepage).
- **`/open`** — unchanged (Create tab's content); the Play/Create tab bar (`HomeTabs.svelte`) lives in the root layout, visible on `/` and `/open` (and `/play/[id]`) whenever `PUBLIC_SHOW_HOME` is true and nothing's loaded, hidden once a game is.
- **`/play/[id]`** — new game-detail route, fetching `api/GameDetails/{id}` (added specifically for this — see below).
