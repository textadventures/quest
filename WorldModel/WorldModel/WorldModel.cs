...
namespace AxeSoftware.Quest
{
    ...
    public class WorldModel : IASL, IASLDebug, IASLTimer
    {
        ...
        internal void UpdateObjectsList(string scope, ListType listType)
        {
            if (UpdateList != null)
            {
                ...
                foreach (Element obj in GetObjectsInScope(scope))
                {
                    ...
                    {
                        objects.Add(new ListData(GetDisplayAlias(obj), obj.Fields[FieldDefinitions.InventoryVerbs], obj.Name));
                    }
                    else
                    {
                        objects.Add(new ListData(GetDisplayAlias(obj), obj.Fields[FieldDefinitions.DisplayVerbs], obj.Name));
                    }
                }
        ...
        private List<ListData> GetExitsListData()
        {
            ...
            {
                exits.Add(new ListData(GetDisplayAlias(exit), exit.Fields[FieldDefinitions.DisplayVerbs], exit.Name));
            }
            return exits;
        }
