...
namespace TextAdventures.Quest.Scripts
{
    ...
    {
        private ExpressionContext _fleeExpressionContext;
        ...
        public ScriptContext(WorldModel worldModel, bool initialiseExpressionContext = false)
        {
            ...
            {
                InitialiseFleeExpressionContext();
            }
        }

        private void InitialiseFleeExpressionContext()
        {
            _fleeExpressionContext = new ExpressionContext(WorldModel.ExpressionOwner);
            _fleeExpressionContext.Imports.AddType(typeof(StringFunctions));
            _fleeExpressionContext.Imports.AddType(typeof(Math));

            _fleeExpressionContext.Variables.ResolveVariableType += Variables_ResolveVariableType;
            _fleeExpressionContext.Variables.ResolveVariableValue += Variables_ResolveVariableValue;
            _fleeExpressionContext.Variables.ResolveFunction += Variables_ResolveFunction;
            _fleeExpressionContext.Variables.InvokeFunction += Variables_InvokeFunction;
            _fleeExpressionContext.Options.ParseCulture = System.Globalization.CultureInfo.InvariantCulture;
        }
        ...
        private void Variables_ResolveFunction(object sender, ResolveFunctionEventArgs e)
        {
            ...
            var proc = WorldModel.Procedure(e.FunctionName);
            if (proc != null)
            {
        ...
        private void Variables_InvokeFunction(object sender, InvokeFunctionEventArgs e)
        {
            ...
            e.Result = WorldModel.RunProcedure(e.FunctionName, parameters, true);
        }
        ...
        private object ResolveVariable(string name)
        {
            ...
            if (WorldModel.TryResolveExpressionElement(Utility.ResolveElementName(name), out var result))
            {
                return result;
            }
        ...
        private void ResolveVariableName(string name, out Fields fields, out string variable)
        {
            ...
            if (WorldModel.TryResolveExpressionElement(name, out var result))
            {
        ...
        public ExpressionContext FleeExpressionContext
        {
            get
            {
                if (_fleeExpressionContext == null) InitialiseFleeExpressionContext();
                return _fleeExpressionContext;
            }
        }
