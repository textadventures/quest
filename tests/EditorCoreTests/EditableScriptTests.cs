...
public class EditableScriptTests : EditorControllerTestBase
{
    ...
    public void TestIfThenElseIf()
    {
        ...
        var newScripts = Controller.CreateNewEditableScripts("game", "somescript",
            "if (someExpression) { msg (\"Then script\") }", true);
        ((EditableIfScript) newScripts[0]).AddElseIf();
        var newElseIf = ((EditableIfScript) newScripts[0]).ElseIfScripts.First();
        ...
        var initialExpectedDisplayString =
            "If (someExpression) Then (Print \"Then script\"), Else If (elseIfExpression) Then (Print \"test\")";
        ...
        var newerExpectedDisplayString =
            "If (someExpression) Then (Print \"Then script\"), Else If (newElseIfExpression) Then (Print \"test\" / Print \"test2\")"
