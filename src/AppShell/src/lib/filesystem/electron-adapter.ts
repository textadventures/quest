...
// PlayCatalog.svelte's file picker (via local-play.ts's playElectronFile).
// Kept as separate lists on the main-process side (see ElectronApp's
// recent-games.ts) precisely so a game opened to play never shows up in
// File > Open Recent (which loads into the editor).
...
export class ElectronFileAdapter implements FileAdapter {
    ...
    // local-play.ts and editor-store.ts's previewInWasmPlayer). window
...
// kind defaults to "edit" for the /open (editor) callers; local-play.ts
