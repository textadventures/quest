<script lang="ts">
    ...
    let versionLine = $derived(
        links?.version ? (links.releaseDate ? t("downloadButton.versionReleased", { version: links.version, date: links.releaseDate }) : links.version) : null,
    );
        ...
        <div class="flex flex-col items-center gap-1 text-sm">
            ...
            {#if links.others.length > 0 || !links.primary}
                <button
                    ...
                    onclick={() => (expanded = !expanded)}
                >
                    {links.primary ? t("downloadButton.otherPlatforms") : t("downloadButton.titleGeneric")}
                </button>
            {/if}
