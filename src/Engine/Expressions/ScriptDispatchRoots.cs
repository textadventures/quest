...
// can't trace these paths statically, so we root the public members here. The evaluator's
// constructor calls EnsureRooted, so this applies to every trimmed host (WasmPlayer and
// WasmEditor alike) without each one having to remember to.
//
