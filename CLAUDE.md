...
- Before opening a PR touching `src/AppShell/src/**`, run `node tests/e2e/find-affected-tests.mjs` to see which `appshell_chromium` scripts a coverage-map snapshot recorded as actually exercising the changed files (real V8 execution coverage, not just "was the file loaded" — see `tests/e2e/lib/tracked-chromium.mjs`'s header for why that distinction matters on a route that statically imports its whole component library), then run those against a local dev server. It's advisory only — `tests/e2e/coverage-map.json` is a point-in-time snapshot regenerated by hand (see `build-coverage-map.mjs`), and it can't see anything below the AppShell/WASM boundary.
