...
That line comes from the `UndoTurn` dynamic template, and "Nothing to undo!" comes from the `NothingToUndo` template, so you can reword both like any other [message](/howto/world/changing-templates).
...
A [Pages dialogue](/howto/npcs/dialogue-pages) doesn't have this problem, because each choice runs as a complete turn of its own and starts its own transaction. The game stays saveable throughout, and each choice is a separate undo point.
...
The `start transaction` script command ends the current transaction and starts a new one, named by the text you give it. It isn't offered when you add a script command in the editor, so add it in [code view](/howto/scripting/codeview). This is exactly what Pages uses to make each choice undoable:
...
3. Click "Copy into your game" in the banner, then edit the script. See [Overriding](/advanced-topics/overriding) for more on this.
...
- [undo](/scripts/#undo) and [start transaction](/scripts/#start-transaction) in the script command reference
- [Changing the game's messages](/howto/world/changing-templates) - rewording `UndoTurn` and `NothingToUndo`
