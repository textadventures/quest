...
For the scripting side of types - testing a value's type, converting between them, what an unset attribute does in a condition - see [Values and types](/howto/scripting/null). For creating and applying your own types in the editor, see [Creating and using object types](/advanced-topics/using-inherited-types).
...
and in the [text processor](/howto/world/text-processor) with braces: `The hat costs {hat.price} gold.`
...
On the _Attributes_ tab, a new attribute starts as a string and you pick its type from the dropdown beside its value. [Values and types](/howto/scripting/null) covers each type, how to find out what type a value is with `TypeOf`, and how null behaves.
...
If a type is called [`defaultobject`](/attributes#defaultobject), it applies to every object in the game without anything inheriting it. Core.aslx uses this to set the defaults you never have to think about:
...
There are matching `defaultexit`, `defaultcommand`, `defaultturnscript` and `defaultgame` types. You can override any of these defaults on an individual object in the usual way, or on the default type itself - see [Overriding Core library functions](/advanced-topics/overriding).

## Mutable attributes on inherited types

Most values are replaced wholesale when you assign to them, so inheritance causes no trouble. Lists and dictionaries are different: they can be changed in place by commands like [list add](/scripts#list-add), and the object is pointing at the type's list, not a copy of it.
...
See [Debugging your game](/howto/scripting/debugging-your-game) for the rest of what it does.

## See also

- [Creating and using object types](/advanced-topics/using-inherited-types) - making your own
- [Values and types](/howto/scripting/null) - types in scripts, and null
- [type element](/elements#type) and [attribute reference](/attributes/)
- [Change scripts](/change-scripts) - running a script whenever an attribute changes
