...
| After every turn | A [turn script](/howto/time/time-turns-and-timers) |
| A set number of turns or seconds from now | A [timer or turn timeout](/howto/time/time-turns-and-timers) |
| Whenever a particular attribute changes | A [change script](/reference/attributes/change-scripts) |
...
The list at the bottom of the tab holds the game's [turn scripts](/howto/time/time-turns-and-timers) - scripts that run after every turn the player takes, anywhere in the game. Add one here, and give it a name if you want to be able to enable and disable it during play.
...
Turn scripts run at the very end of a turn, after everything else - including, when the player has moved, after all of the new room's entry scripts. They do not run after a command Quest Viva did not understand; the [Time, turns and timers](/howto/time/time-turns-and-timers#what-counts-as-a-turn) page lists everything else that does not count as a turn.
...
Initialisation scripts run just after the game's start script, in turn for every object in the game. They are useful when the setup for an object belongs with the object rather than in one long start script - particularly for objects defined in a [type](/customise/object-types) or a [library](/customise/libraries), which have no way to add to the game's start script.
...
Turn scripts cover "every turn". For "in three turns' time", "in ten seconds' time" or "every twenty seconds", see [Time, turns and timers](/howto/time/time-turns-and-timers), which covers turn scripts, `SetTurnTimeout`, timers and `SetTimeout`, and what does and does not count as a turn.

## See also

- [Time, turns and timers](/howto/time/time-turns-and-timers)
