...
The first thing to realise is that this is a one way trip. You start creating with Trizbort, then export to Quest Viva, and then create with Quest Viva. You cannot go back to Trizbort once you have started to make changes in Quest Viva.

So with that in mind, the way to approach it is to design the geography in Trizbort, then turn that into a game in Quest Viva.
...
Rooms in Quest Viva have to have unique names; it is a good idea to select Validation – Rooms must have a unique name, so Trizbort will warn you if that is not the case.

Note that Subtitle, Regions and Room shapes will not be exported to Quest Viva.
...
You can make other changes too, but nothing that will get exported into Quest Viva.
...
Note that position just determines where the list appears in Trizbort, and is not exported to Quest Viva.

 

## Map settings

Go to _Tools – Settings_ to see general setting. Here you can give your game a title, add yourself as the author and add a description. This is just as easily done in Quest Viva, and you may prefer to do it there.

None of the other settings will be included in your Quest Viva game.
...
Once you have completed your map, go to _File – Export_ to export your game, and select _Quest_. Navigate to the folder, and click “Save”. The exported file is your new Quest Viva game.
...
You should now be able to open your game in Quest Viva.
...
Quest Viva handles maps very differently to Trizbort. Quest Viva tries to guess where each room is in relation to each other, whilst Trizbort is a drawing package, and allows rooms to go anywhere. Quest Viva uses exits and Trizbort uses links. The upshot is that map itself does not export from Trizbort. The only values that are exported are the colours and size of the room.
