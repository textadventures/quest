...
Add `turns` to the game's [status attributes](/reference/attributes/status) to show it to the player. Because it's a turn script, it only counts turns that really are turns.
