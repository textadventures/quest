...
The file "spinning_compass.ogv" is in my game's main folder. Every file in that folder is included when you publish, so the video is packaged with your game. For more, see [The publish process](/publishing/publishing#the-publish-process).
...
The easiest way to stop a video would be removing ALL video tags from the game.  This can be handled [using Javascript](/howto/ux/customising-the-ui) via `JS.eval()` (NOTE: This will completely remove any HTML audio tags you have added to the game!).
