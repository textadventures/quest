...
It would be good if the player can see at a glance how she is doing. We can use status attributes to do that. The first step is to give the player a string dictionary, called "statusattributes". Then we can add the attributes we want to track. (For a closer look at how status attributes work, see [Awarding points only once](/howto/world/score-health-money#awarding-points-only-once).)
...
To turn on money, tick the "Money" box on the _Features_ tab of the game object, then go to the _Player_ tab of the player object and set the start money, if you want the player to start with any. For more on how money works in Quest Viva, see [Score, health and money](/howto/world/score-health-money), and for buying and selling, see [Setting up a shop](/howto/tasks/shop).
