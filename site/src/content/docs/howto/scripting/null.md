...
On the _Attributes_ tab these are the same types you pick from when you add an attribute. See also [Lists](/howto/scripting/using-lists), [Dictionaries](/howto/scripting/using-dictionaries) and, for the difference between `int` and `double`, [Maths](/howto/tasks/use-maths-functionality).
...
`HasAttribute` counts attributes the object gets from its [types](/advanced-topics/using-inherited-types) as well as its own. If you need to know whether this particular object has its own copy, use `GetAttributeNames(obj, false)`, which lists only an object's own attributes - `GetAttributeNames(obj, true)` includes inherited ones.
...
- [Writing code](/howto/scripting/introtocoding) - expressions, operators and the common mistakes
- [Attribute functions](/reference/functions/attributes) - the full `Get`/`Has`/`Set` reference
- [Maths](/howto/tasks/use-maths-functionality) - rounding, and choosing between `int` and `double`
- [Using inherited types](/advanced-topics/using-inherited-types) - where an attribute's fallback value comes from
- [The Debugger](/howto/scripting/debugging-your-game) - see what every attribute actually holds while the game runs
