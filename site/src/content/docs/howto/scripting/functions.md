...
Quest Viva already has a large number of functions built in, listed in the [function reference](/reference/functions/). It also has [script commands](/reference/script-commands/), which look similar but are written in lower case (`msg`, `list add`, `firsttime`) and can't be used in an expression. When you pick "Print" in the script editor, you are adding the `msg` script command.
...
| Different behaviour on each of several objects | A [verb](/howto/commands/verbs) or script attribute on each object |
| The same behaviour on every object of a kind | A [type](/customise/object-types) with the script on it |
...
A function that returns a value can be used anywhere an expression can, including inside another function call, inside a condition, or in the [text processor](/howto/text/text-processor):
...
For something you will want to check more than once, a [walkthrough](/howto/testing/walkthroughs) with `assert:` lines does the same job automatically every time you run it, and the [Debugger](/howto/testing/debugging) lets you watch attributes change as you play.

## See also

- [Script commands](/reference/script-commands/) and the [function reference](/reference/functions/)
- [Introduction to coding](/howto/scripting/writing-code)
- [Using delegates](/customise/delegates), for storing a script on an object and calling it like a function
