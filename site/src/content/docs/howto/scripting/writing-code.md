...
| Rewrite a whole element, or several at once | [The raw XML view](/howto/scripting/raw-xml) |
...
- **Script commands** such as `msg`, `foreach`, `set` and `list add`. These are built into the language, and you'll find them all in the [script command reference](/reference/script-commands/).
- **Functions** such as `GetBoolean`, `MoveObject` and `Split`. These are ordinary functions - the engine ships hundreds, and you can write your own. See the [function reference](/reference/functions/) and [Functions](/howto/scripting/functions).
...
- **`game.pov`** is the current player object. Quest Viva can [switch the player character](/howto/player/player-object) mid-game, so library code always says `game.pov` rather than `player`. In your own game, `player` is fine until the day you add a second player character.
...
Text you print also goes through the [text processor](/howto/text/text-processor), so you can put values into the text directly rather than concatenating:
...
| `'obj.att' is null (it has not been set) and cannot be used in this calculation` | Arithmetic on an attribute that was never set - see [Values and types](/understanding/values-and-types) |
...
The [Debugger](/howto/testing/debugging) will show you what every attribute actually holds while the game is running, which is normally the fastest way to find out why a condition isn't firing.

## See also

- [Script commands](/reference/script-commands/) and [Functions](/reference/functions/) - the full reference
- [Values and types](/understanding/values-and-types) - what a value can be, and what happens when an attribute isn't set
- [Functions](/howto/scripting/functions) - writing your own
- [When scripts run](/understanding/when-scripts-run) - which script fires when
- [Editing the raw XML](/howto/scripting/raw-xml) - the whole game file as text
