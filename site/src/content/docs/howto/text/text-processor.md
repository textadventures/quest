...
- [templates and dynamic templates](/howto/text/messages) - the built-in messages use directives themselves
...
| `{either condition:text}`<br/>`{either condition:text if true\|text if false}`<br/>`{either condition:text if true:text if false}` | Like `{if}`, but the condition is a full [expression](/howto/scripting/writing-code#expressions-and-operators), so it can use functions, `and`, `or` and `not`, and compare strings with double quotes |
...
| `{rndalt:object}` | Prints a name picked at random from the object's [alt](/reference/attributes/all#alt) list |
...
- [Changing the game's messages](/howto/text/messages) - the built-in text, which is written with these directives
- [Expressions](/howto/scripting/writing-code#expressions-and-operators) - what you can put in `{either}` and `{=}`
- [Pictures](/howto/media/pictures) - more on `{img:}` and the picture frame
