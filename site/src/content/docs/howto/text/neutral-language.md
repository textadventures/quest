...
Quest Viva has attributes and functions for exactly this. The text processor can't do any of it, so you build the message as an [expression](/howto/scripting/writing-code#expressions-and-operators) - in the editor, add a "Print a message" script and change its dropdown from "message" to "expression".
...
Quest Viva's own messages are written this way. `WriteVerb(game.pov, "can't")` gives "You can't" - see [Changing the game's messages](/howto/text/messages) for how to change them.
