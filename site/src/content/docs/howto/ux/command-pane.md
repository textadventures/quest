...
Let us suppose we want to add a SHOUT command to a specific room (see [here](/howto/commands/commands-for-room) for the best way to do that). Go to the _Scripts_ tab of the room, and add this to the script that runs when the player enters the room (not the one that only runs the first time):
