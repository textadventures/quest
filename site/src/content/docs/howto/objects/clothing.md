...
Clothing is a built-in feature. Tick "Wearable: object can be put on and taken off" on the object's [_Features_ tab](/howto/objects/features#object-features), then go to the _Wearable_ tab that appears and change the dropdown from "Cannot be worn" to "Can be worn".
...
Tick "Show advanced options for wearables" on the game object's [_Features_ tab](/howto/objects/features#game-features) and the _Wearable_ tab gains:
...
| Protection (if implemented!) | The `armour` attribute, for a game with its own combat system. See [RPG games](/howto/rpg/designing-an-rpg). |
...
[Override](/customise/overriding) `TestGarment` to veto putting something on. It takes the garment and returns a boolean - print a message and return false to refuse:
...
If you use [inventory limits](/howto/objects/taking-and-dropping#inventory-limits), wearing something raises the player's limit by one and removing it lowers it again, so worn items effectively do not count. Volume limits are unaffected - the player is still carrying the weight.

To keep worn clothes out of the main inventory listing, create a [container](/howto/objects/containers) on the player object, give it a boolean attribute `wornclothinglocation` set to true, and untick "Drop" on its _Inventory_ tab. Anything worn goes inside it, and closing the container hides the clothes from the listing.
