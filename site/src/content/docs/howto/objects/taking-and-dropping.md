...
One difference is worth knowing. A drop script is passed a `destination` parameter — the place the object would have gone, which is normally the player's current room but can be redirected (see [drop destinations](/howto/rooms/objects-and-rooms#the-room-tab)). Use it rather than assuming `player.parent`:
