...
If you have several containers, give each one `water` and `capacity` attributes and the same verbs. Once you're doing that for more than a couple of objects, it's worth [creating a type](/customise/object-types) that holds the attributes and verb scripts, and having each container inherit it.
...
- [Turning one thing into another](/howto/objects/transforming)
