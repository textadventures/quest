...
The [Tutorial](/tutorial/using-pages) covers the basics of setting up a Pages dialogue tree. This page covers some more advanced techniques once you've got the hang of it.

## When to use Pages

Quest Viva has several ways to let the player talk to a character - see [Talking to characters](/howto/characters/talking) for an overview. Pages are the right choice when you want a structured, multi-step exchange where each reply leads to a fixed set of further choices - the Text Adventure equivalent of a gamebook's branching passages. For a one-off list of topics with no follow-up, a [menu](/howto/characters/talking#a-menu-of-topics) is simpler. For a free-form "ask about anything" system, use [Ask/Tell](/howto/characters/ask-tell) instead.

Each choice is a complete, ordinary turn, so the player can save at any point in a conversation - see [Menus, Pages and saving](/howto/characters/talking#menus-pages-and-saving) for how this compares with menus. (The player cannot type `UNDO` while the dialogue is waiting, because what they type is taken as their choice.)
...
Under the hood, options are stored in the page's `options` attribute, a [stringdictionary](/reference/attributes/types#stringdictionary) keyed by the destination page's object name - `AddPageLink`/`RemovePageLink` are just a convenient wrapper around `dictionary add`/`dictionary remove` on that attribute, so you can fall back to those directly for anything more unusual.
