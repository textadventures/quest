...
For topic lists, `ASK ABOUT` without a character, and other extensions, see [Building an Ask/Tell system](/howto/characters/ask-tell).
...
For changing a conversation's options as the game goes on, redirecting to a different page from a script, and more, see [Building a conversation with Pages](/howto/characters/pages).
...
See [The text processor](/howto/text/text-processor) for more.
...
Conversations are only one part of bringing a character to life. [Characters that move](/howto/characters/moving) shows how to make characters follow the player, patrol a route, wander at random or go somewhere with a purpose.
