...
The score belongs to the `game` object, so it stays the same if the player [switches to a different character](/howto/player/player-object). It is always zero at the start of the game, even if you've given the `game` object a `score` attribute of your own.
...
Every script that awards "Crossing the chasm" can call `AwardPoints` with the same name, and the player only gets the points once. For more about functions and parameters, see [Functions](/howto/scripting/functions).
...
Health belongs to the player object - if you have [more than one player object](/howto/player/player-object), each has its own health, and the status pane shows the current one's. Health is a percentage. It starts at 100, can't go above 100 and can't go below 0: if a script sets it higher or lower, Quest Viva brings it back into range.
...
Food, potions and the like can restore health. Make the object edible, then set "Change health by" on its _Edible_ tab - see [Items that can be eaten](/howto/objects/food).

If you need a different maximum, such as hit points that go up as the player gains levels, don't tick "Health". Use your own attribute with a different name, and show it as a [status attribute](/reference/attributes/status). [Designing an RPG](/howto/rpg/designing-an-rpg) covers that choice in more detail.
...
For a worked shop, with a stockroom the player can't reach and BUY, BROWSE and SELL commands, see [Setting up a shop](/howto/score/shop).
...
Your own values go in the same lists, and don't hit the bug. [Status attributes](/reference/attributes/status) covers the format strings in full, including HTML and "out of" totals like `Score: !/100`.
...
- [Status attributes](/reference/attributes/status) - showing any value in the status pane
- [Setting up a shop](/howto/score/shop) - buying and selling with the money feature
- [Designing an RPG](/howto/rpg/designing-an-rpg) - stats, levels and combat, where the built-in health feature isn't enough
