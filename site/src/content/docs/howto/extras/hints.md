...
- `SuppressTurnscripts` stops asking for a hint from counting as a turn, just like Core's `HELP` - see [Commands that shouldn't take a turn](/howto/time/time-turns-and-timers#commands-that-shouldnt-take-a-turn).
...
Play through your game, typing `HINT` at every stage, to check that each hint appears when it should. A misspelt object name in a `game.hintstage` line only causes an error when that line runs, so it's easy to miss. A [walkthrough](/howto/testing/walkthroughs) that includes `HINT` at each stage catches this every time you run it.
