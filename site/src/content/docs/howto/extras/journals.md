...
`#text#` matches anything the player types, and puts it in a variable called `text` (see [Commands](/howto/commands/how-commands-work)). The last pattern means that any line starting with a dash is treated as a note, so the player can type `-the guard is lying` without typing a command at all. The script is:
...
- [Using lists](/howto/scripting/lists) and [Using dictionaries](/howto/scripting/dictionaries)
