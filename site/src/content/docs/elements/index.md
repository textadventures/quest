...
You can run delegate functions on objects using the [rundelegate](/scripts#rundelegate) command (if the delegate does not return a value) or using the [RunDelegateFunction](/reference/functions/general#rundelegatefunction) function (for delegates that do return a value).
