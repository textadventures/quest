...
All of these are on the "game" element; see [Look and feel](/customise/look-and-feel) for every option.
...
With hyperlinks on, the objects and exits in room descriptions are links. Clicking an object shows a menu of its [display verbs](/customise/object-verbs), such as "Look at" and "Take"; clicking an exit goes that way. You can add your own links anywhere the text processor works, such as a room description or a message:

```quest
msg ("The guard looks bored. Perhaps you could {command:wait:wait for him to leave}.")
```

A `{command:...}` link runs the command as if the player had typed it. See [Links](/howto/text/text-processor#links) for the other kinds.

## Panes

The panes list what the player is carrying and what's in the room, with buttons for each object's verbs, and a compass for moving around. A game can also show a Status pane, a command pane of one-click commands and a custom status pane - see [Panes](/customise/panes).
...
- **Links give things away.** A list of verbs tells the player what's possible. Typing avoids that, at the risk of "guess the verb" frustration. You can hide or add verbs on each object's [Object tab](/customise/object-verbs).
...
| To change | Read |
|---|---|
| The theme, fonts, colours, panes, command bar and room descriptions | [Look and feel](/customise/look-and-feel) |
| Which verbs appear for an object, in menus and panes | [Object verbs](/customise/object-verbs) |
| The command pane and custom status pane | [Panes](/customise/panes) |
| Anything the options don't cover, with your own CSS | [Styling the player](/customise/css) |
| The interface's behaviour, with JavaScript | [Calling the game from JavaScript](/customise/javascript) |
