...
| You want | Try first |
|---|---|
| Different wording for a standard message | [Change the template](/howto/text/messages) - the same mechanism, already documented |
...
| Different behaviour for a group of objects | A [type](/customise/object-types) with the behaviour on it |
| Something to run whenever an attribute changes | A [change script](/reference/attributes/change-scripts) |
...
- **Templates** hold every standard message in the game, so they're the ones you're most likely to want. [Changing the game's messages](/howto/text/messages) covers them properly, including varying a response at random.
...
- **The `defaultobject` type** is worth knowing about. Every object in the game inherits it, so copying it into your game and adding an attribute gives that attribute to every object at once - see [Attributes and types](/understanding/attributes-and-types#default-attributes-for-every-object).

## What can't be overridden

Not everything Quest Viva does is written in Quest script. Some functions are part of the engine itself, written in C#: `CapFirst`, `ToInt`, `ListCount`, `TypeOf`, `GetBoolean`, `IsRegexMatch` and many others, plus every [script command](/reference/script-commands/) (`msg`, `if`, `foreach`, `list add`).
...
- [Creating functions](/howto/scripting/functions) - writing your own from scratch
- [Using libraries](/customise/libraries) - packaging your changes for reuse across games
- [Editing the raw XML](/howto/scripting/raw-xml) - reading Core's source directly
- [Attributes and types](/understanding/attributes-and-types) - what `defaultobject` does
