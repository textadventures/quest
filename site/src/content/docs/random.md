...
Not really a function, but the easiest to use. As with all [text processor](/text_processor) directives, this is embedded in a string. The directive is called "random", and will select one text from the following list.
...
Now we have a clone with a description that matches its attributes (this is also discussed on the [clones](/clones) page).
