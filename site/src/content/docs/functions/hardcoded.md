...
Quest functions fall into two types, hard-coded and library. Generally when you are creating a text adventure, you will see no difference, but just occasionally it might be significant.

### Library Functions

Most of the Quest functions are written in Quest's own coding language. These library functions are just like functions you create yourself, the only difference is that the work has been done for you. 
...
Hard-coded functions will not be found in .aslx files and will not appear in the list if you show library elements. Quest functions are usually strict about the number of parameters, but C# allows optional parameters, so some hard-coded functions have optional parameters. Others are just as strict - but will produce different error messages if they do not like the number of parameters you sent them.
