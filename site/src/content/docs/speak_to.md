...
This could then be handled with a stock response or by offering a list of options (Quest will accept `TALK TO` as a synonym of `SPEAK TO`). This is a restricted approach to conversations, compared to [ASK/TELL](/ask_about), because while the player can choose who to talk to, you are dictating what the conversation is about.
