...
Tick the _Multistate?_ box to show options related to garments that can be in more than one state (such as a jacket that can be open or fastened), as described on [this page](/multistate-clothing).
...
Finally, you need to [override](/overriding) the `TestGarment` function.
