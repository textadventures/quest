...
Embeds an autoplaying YouTube video for the given video ID. See [Sound and video](/howto/media/sound-and-video#a-youtube-video).
...
Clears everything drawn on the map grid - rooms and any custom layers. Used when resetting the map entirely, e.g. when the player teleports to an unconnected region. See [Showing a map](/howto/rooms/map).
...
Sets the commands to be displayed on the command pane (tick "Show a command pane" on the game object's _Interface_ tab). See [Panes](/customise/panes#command-pane). Commands should be sent as a string, separated by semi-colons. The colour of the text can be specified, but is optional.
...
Sets the HTML text to be displayed on the custom status pane (tick "Show a custom status pane" on the game object's _Interface_ tab). See [Panes](/customise/panes) for an example.
...
Shows or hides the status variables pane (see [status attributes](/reference/attributes/status)).
