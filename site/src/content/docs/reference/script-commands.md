...
An initial type can be specified e.g. "northdirection". This will ensure that the correct [alt](/reference/attributes/all#alt) names are applied to compass exits.
...
For more on how and why to use `foreach`, see [Using Lists](/howto/scripting/lists)
...
Note that this does not wait for scripts attached to functions to work (such as `Ask` and `ShowMenu`). see [Saving while a question is waiting](/howto/scripting/asking-the-player#saving-while-a-question-is-waiting)
...
| `GameName` | Sets the name of the game. | [JS.setGameName(name)](/reference/js/#setgamename) |
| `Hide` | Turns off an interface element. | [JS.uiHide(...)](/reference/js/#uihide) |
...
This command should only be used within a [\<function\> element](/reference/elements#function).
...
If the "allow cancel" parameter is set to **true**, entering any command other than one of the option numbers dismisses the menu, and the variable "result" is [null](/reference/attributes/types#null). If it is set to **false**, anything else the player types is ignored.
