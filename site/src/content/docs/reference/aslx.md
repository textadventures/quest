...
-   [English.aslx](/customise/other-languages) provides the English text for the built-in default responses, and the names of the verbs whose behaviour is defined in Core.aslx. This means Core.aslx is language-neutral – if you wanted to make a game in German or Spanish, just translate English.aslx and plug it in to your game.

## Attributes

Each object's attributes are defined in the XML. Attributes define all the behaviour of an object. The XML "type" attribute used is to specify the type of the attribute. If no type is specified, the [string](/reference/attributes/types#string) type is assumed, as with the sink's "look" attribute in the above example. An exception is if there is no data in the XML tag, in which case a [boolean](/reference/attributes/types#boolean) "true" is assumed instead.

The available types are listed on the [Attribute Types](/reference/attributes/types/) page.
