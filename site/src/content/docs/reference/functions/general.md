...
Returns the [int](/reference/attributes/types#int) value of the current Unix epoch time in UTC. 
...
Loads the specified file and returns a [string](/reference/attributes/types#string) containing its contents.
...
Returns a [string](/reference/attributes/types#string) containing the full path to the specified file. The file must exist in the same directory as the game. This can be used to access a game's resources such as sounds and pictures, and pass their URLs to the player UI.
...
Writes the [string](/reference/attributes/types#string) **text** to the browser's JavaScript/developer console. This is intended for debugging during game development, not as a player-facing feature.
