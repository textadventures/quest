...
Status attributes do not support the text processor, but they are displayed in HTML, so you can add fancy formatting.
```xml
Total score: <b>!/10</b>
```
...
The attribute can be added to the list of status attributes as before, or from the game start script:

```quest
dictionary add (player.statusattributes, "ammonote", "Ammo: !")
```

So how do we ensure `player.ammonote` always gets updated? With [change scripts](/reference/attributes/change-scripts)...



## Behind the scenes

Core.aslx updates the status attributes in [UpdateStatusAttributes](/reference/functions/user-interface#updatestatusattributes). It builds a string list from each of the status attribute dictionaries in turn and sends the joined result to the UI with `JS.updateStatus`. (Quest 5 used a `SetStatus` [request](/reference/script-commands#request) for this; that route still exists but is no longer what Core uses.)
