...
Attributes help to describe an object to the game world and users of Quest Viva, attributes can be added to an [object](/reference/attributes/types#object) and can be of any type supported by Quest Viva.
...
See the [Custom attributes](/tutorial/custom-attributes) tutorial for more information.
...
[Status attributes](/reference/attributes/status) are just attributes that you tell Quest Viva to display - there is nothing special about the attribute itself. 


## Change scripts

A [change script](/reference/attributes/change-scripts) is a special attribute, a script that will fire whenever another attribute changes. From the example above, we want to update `player.ammonote`, whenever `player.ammo` changes (and `player.ammomax` too).
...
There are several [important attributes](/reference/attributes/important) that already have a meaning in Quest Viva, and you are probably best avoiding them to avoid confusion.
