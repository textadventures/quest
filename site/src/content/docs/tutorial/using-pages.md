...
Pages need something to kick them off. Go to Bob's Verbs tab and add a "speak" verb (Quest Viva will match `TALK TO BOB` and `SPEAK TO BOB` to it - see [Talking to characters](/howto/characters/talking#responding-to-speak-to) for more on this). For its script, switch to Code View and enter:
...
For more on building larger dialogue trees - including how to add and remove options while the game is running - see [Building a conversation with Pages](/howto/characters/pages).
